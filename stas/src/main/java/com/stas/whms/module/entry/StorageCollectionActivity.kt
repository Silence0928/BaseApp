...
class StorageCollectionActivity : BaseMvvmActivity<ActivityStorageCollectionBinding, BaseViewModel>() {
    ...
    private fun handleBindData() {
        ...
        val tableData: TableData<UserInfo> =
            TableData<UserInfo>("用户信息", userList, coId, coName, coAge, coPhone, coSex, coDel)
            ...
            object : BaseCellBackgroundFormat<CellInfo<*>?>() {
                override fun getBackGroundColor(cellInfo: CellInfo<*>?): Int {
                    return if (cellInfo?.row.let { it!! }.toInt() % 2 != 0) {
                        ContextCompat.getColor(this@StorageCollectionActivity, com.lib_src.R.color.green_trans_22)
                    } else TableConfig.INVALID_COLOR
                }

                override fun getTextColor(t: CellInfo<*>?): Int {
                    return if (t?.col == 5) ContextCompat.getColor(this@StorageCollectionActivity, com.lib_src.R.color.blue11) else
                        ContextCompat.getColor(this@StorageCollectionActivity, com.lib_src.R.color.black04)
                }
            }
